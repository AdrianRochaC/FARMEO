...
app.post('/api/ai/generate-questions', verifyToken, async (req, res) => {
  ...
  try {
    ...
    try {
      ...
      console.error('Error específico de IA:', aiError.message);
      // Fallback a preguntas básicas si falla OpenAI
      if (aiError.message.includes('API key') || aiError.message.includes('OPENAI_API_KEY')) {
        return res.status(503).json({
          success: false,
          message: 'Servicio de IA no configurado (Falta API Key). Por favor contacta al administrador.',
...
app.post('/api/ai/analyze-video-file', videoAnalysisUpload.single('videoFile'), verifyToken, async (req, res) => {
  ...
  try {
    ...
    // Manejar errores específicos de configuración o herramientas
    if (error.message.includes('ffmpeg') || error.message.includes('not found')) {
      return res.status(503).json({
        success: false,
        message: 'El procesador de video (FFmpeg) no está instalado en el servidor. El administrador debe instalarlo para usar esta función.',
